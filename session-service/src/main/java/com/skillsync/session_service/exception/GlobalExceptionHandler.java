package com.skillsync.session_service.exception;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorDetails> handleValidationException(MethodArgumentNotValidException ex) {
        ...
        ErrorDetails errorDetails = new ErrorDetails(LocalDateTime.now(), "Validation Failed", msg);
        return new ResponseEntity<>(errorDetails, HttpStatus.BAD_REQUEST);
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<ErrorDetails> handleGlobalException(Exception exception, WebRequest webRequest) {
        System.err.println("CRITICAL ERROR in Session Service: " + exception.getMessage());
